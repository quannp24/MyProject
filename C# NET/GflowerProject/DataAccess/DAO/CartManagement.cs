...
namespace DataAccess.DAO
{
    public class CartManagement
    {
        ...
        public async Task<List<Cart>> GetCarts()
        {
            ...
            {
                var flowerDB = new GFlowersContext();
                carts = await flowerDB.Carts.Include(c => c.Product).AsNoTracking().ToListAsync();
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
        ...
        public async Task<Cart> FindCartById(int id)
        {
            ...
            try
            {
                var context = new GFlowersContext();

                cart = await context.Carts.AsNoTracking().FirstOrDefaultAsync(e => e.CartId == id);
