...
import { faSquarePlus, faTrash, faXmark, faShareFromSquare, faTrashCan, faFloppyDisk, faCheck } from '@fortawesome/free-solid-svg-icons';
...
@Component({
  ...
  styleUrls: ['./approve-question.component.scss']
})
export class ApproveQuestionComponent {
  radioText: any[] = [];
  questionList: any[] = [];
  ...
  */
  SendRequestApprove(question: any, status: number) {
  ...
  getQuestionsByCategory() {
    this.quizDataService.getQuestionsByCategory(this.categoryId, 2, undefined).subscribe(res => {
  ...
  */
  rejectQuestion() {
    this.SendRequestApprove(this.questionSelect, 4)
    this.displayDialog = false;
  }

  approve(question: any) {
    this.SendRequestApprove(question, 1)
    this.displayDialog = false;
  }
  /*
 ...
 */
  ...
  */
  ...
  showDeleteConfirmation(question: any): void {
    this.questionId = question.quesId
